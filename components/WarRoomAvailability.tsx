...
interface WarRoomAvailabilityProps {
  ...
  deptName: string;
  selectedDate: string;
  timeMode: 'day' | 'month' | 'quarter' | 'year'; // Added 'day'
  selectedYear: string;
...
const WarRoomAvailability: React.FC<WarRoomAvailabilityProps> = ({ deptId, deptName, selectedDate, timeMode }) => {
  ...
  const theoreticalDays = useMemo(() => {
    ...
    if (timeMode === 'month') return 22; // Month
    return 1; // Day
  }, [timeMode]);
              ...
              <div className="flex-1 min-w-0">
                <div className="flex justify-between items-start mb-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-[8px] font-bold px-1 py-0.5 rounded ${item.status === 'Optimal' ? 'bg-brand-green/10 text-brand-green' :
